#!/usr/bin/env python3
"""Fix idiom database v3 — handles corruption inside fibonacciSphere function."""
...
# Step 1: Fix the fibonacciSphere function.
# Lines 14-33 should be:
#   function fibonacciSphere(...): [number, number, number] {
#     const phi = ...;
#     const theta = ...;
#     return [
...
#     ];
#   }
# But currently lines 22-31 have idiom entries injected into the return [].
# Fix: replace the broken function.

# Find the fibonacciSphere function
fn_start = None
fn_end = None
for i, line in enumerate(lines):
    if 'function fibonacciSphere' in line:
        fn_start = i
    if fn_start and i > fn_start and line.strip() == '}' and 'fibonacci' not in lines[i-1]:
        # Check if this is the right closing brace (one-liner function, then a blank or next section)
        # Actually let's find it more precisely
        pass
...
# The function body contains idiom entries. Let's find where it should end.
# Look for "const GALAXY_RADII" which comes right after
fn_end = None
for i in range(fn_start, len(lines)):
    if 'const GALAXY_RADII' in lines[i]:
        fn_end = i - 1  # the line before GALAXY_RADII
        break

print(f"fibonacciSphere: lines {fn_start}-{fn_end}")
...
lines[fn_start:fn_end+1] = correct_fn
print("Fixed fibonacciSphere function")

# Step 2: Now find where to insert the new idioms for each galaxy.
# The file has:
#   - HISTORY: 20 entries (lines after "历史典故" comment)
#   - MYTHOLOGY: 20 entries
#   - NATURE: 20 entries
#   - PHILOSOPHY: 20 entries
#   - EMOTION: 30 entries (20 original + 10 new at the end)

# New idiom entries (from fix_idioms2.py)
new_idioms = {
    'HISTORY': [
...
# Map section names to galaxy keys and next section names
section_map = {
    '历史典故': ('HISTORY', '神话传说'),
    '神话传说': ('MYTHOLOGY', '自然风物'),
    '自然风物': ('NATURE', '为人处世'),
    '为人处世': ('PHILOSOPHY', '情感心理'),
    '情感心理': ('EMOTION', None),  # None means insert before `  ];`
}

# Step 3: Remove the extra EMOTION entries that are at the end (they'll be re-inserted)
# Find the lines with the new entries at the end of EMOTION section
# These are the entries from 魂牵梦萦 to 黯然神伤 that appear AFTER 热血沸腾
remove_start = None
for i, line in enumerate(lines):
    if '热血沸腾' in line and 'GalaxyCategory.EMOTION' in line:
        remove_start = i + 1
        break

if remove_start:
    # Find the closing `  ];` after remove_start
    remove_end = None
    ...
    if remove_end:
        ...
        n_removed = sum(1 for l in removed if '{ text:' in l)
        print(f"Removing {n_removed} extra entries from end of EMOTION section (lines {remove_start}-{remove_end})")
        del lines[remove_start:remove_end]
        # Now we have 20 entries per galaxy in HISTORY/MYTHOLOGY/NATURE/PHILOSOPHY/EMOTION
        # (the 10 extra EMOTION entries that were added are now removed too)

# Step 4: Insert new entries in correct positions
# For each section (except EMOTION), insert before the next section's comment
# For EMOTION, insert before `  ];`

# Recalculate line numbers after modifications
offset = 0
for section_name in ['历史典故', '神话传说', '自然风物', '为人处世']:
    galaxy, next_name = section_map[section_name]
    ...
    if insert_at:
        new_entries = new_idioms[galaxy]
        for entry in reversed(new_entries):
            lines.insert(insert_at, entry)
        print(f"Inserted {len(new_entries)} entries for {galaxy} ({section_name}) before '{next_name}' comment")
...
new_entries = new_idioms['EMOTION']
for entry in reversed(new_entries):
...
new_content = new_content.replace('在球面上均匀分布 100 个位置', '在球面上均匀分布 150 个位置')
new_content = new_content.replace('生成 100 个成语', '生成 150 个成语')

# Fix connection limits (>= 200 → >= 300)
