#!/usr/bin/env python3
"""Fix idiom database - properly redistribute 150 idioms across 5 galaxies."""

# Read current file
with open('src/data/idioms.ts', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Find the rawIdioms array boundaries
array_start = None
array_end = None
for i, line in enumerate(lines):
    if 'const rawIdioms' in line and '= [' in line:
        array_start = i
    if array_start and i > array_start and '  ];' in line:
        array_end = i
        break

print(f"Array spans lines {array_start} to {array_end}")

# Find galaxy section boundaries within the array
galaxy_lines = {}
for g, name in [('HISTORY', '历史典故'), ('MYTHOLOGY', '神话传说'),
                ('NATURE', '自然风物'), ('PHILOSOPHY', '为人处世'),
                ('EMOTION', '情感心理')]:
    for i in range(array_start, array_end):
        if name in lines[i] and '═══' in lines[i]:
            galaxy_lines[g] = i
            break

print("Galaxy section lines:", galaxy_lines)
...
section_order = ['HISTORY', 'MYTHOLOGY', 'NATURE', 'PHILOSOPHY', 'EMOTION']
next_section_marker = {
    'HISTORY': '神话传说',
    'MYTHOLOGY': '自然风物',
    'NATURE': '为人处世',
...
# First, remove ALL extra entries that were incorrectly added (keep only original 100)
# The original file has entries up to line ~153 (end of original EMOTION section)
# Everything after the last original EMOTION entry but before `  ];` is extra

# Find the `  ];` line that closes rawIdioms
raw_end_line = None
for i, line in enumerate(lines):
    if i > array_start and '  ];' in line.strip() and i > 150:
        raw_end_line = i
        break

print(f"rawIdioms closing bracket at line {raw_end_line}")

# Build new lines: keep everything before and including the last original entry,
# then add new entries for each galaxy, then close

# We need to insert new entries right before `  ];`
# First, find which line is the last original idiom entry
last_original_line = raw_end_line - 1
while last_original_line > 0:
    if 'galaxy: GalaxyCategory' in lines[last_original_line]:
        break
    last_original_line -= 1

print(f"Last original idiom at line {last_original_line}: {lines[last_original_line][:80]}...")

# Remove all lines between last original entry + 1 and raw_end_line
# These are the incorrectly placed entries
del lines[last_original_line + 1:raw_end_line]
# raw_end_line is now different; recalculate
raw_end_line = last_original_line + 1

# Now insert new idioms before `  ];` (which is now at raw_end_line)
# But we need to insert them in the RIGHT sections
# Actually, let's find each section boundary and insert there

# Find the last line of each section (before the next section's comment)
for galaxy in ['HISTORY', 'MYTHOLOGY', 'NATURE', 'PHILOSOPHY']:
    next_name = next_section_marker[galaxy]
    # Find the comment line for the next galaxy
    insert_point = None
    for i in range(array_start, raw_end_line):
        if next_name in lines[i] and '═══' in lines[i]:
            insert_point = i
            break
    if insert_point:
        # Insert before the comment, after a blank line
        # Find the last entry before this comment
        j = insert_point - 1
        while j > array_start and lines[j].strip() == '':
            j -= 1
        # j is now the last entry line of the current section
        # Insert after j+1
        new_lines = new_idioms[galaxy]
        for k, entry in enumerate(new_lines):
            lines.insert(j + 1 + k, entry)
        # Update raw_end_line since we inserted lines
        raw_end_line += len(new_lines)
        print(f"Inserted {len(new_lines)} entries for {galaxy} at line {j+1}")

# EMOTION goes before `  ];`
# Find the closing bracket now
raw_end_line = None
for i, line in enumerate(lines):
    if i > array_start and '  ];' in line.strip() and i > 150:
        raw_end_line = i
...
print(f"New closing bracket at line {raw_end_line}")

# Insert EMOTION entries before `  ];`
for k, entry in enumerate(new_idioms['EMOTION']):
    lines.insert(raw_end_line + k, entry)

print(f"Inserted {len(new_idioms['EMOTION'])} EMOTION entries")

# Update section comments and header
content = ''.join(lines)
...
content = content.replace('connections.length >= 200\n', 'connections.length >= 300\n')
...
for g in ['HISTORY', 'MYTHOLOGY', 'NATURE', 'PHILOSOPHY', 'EMOTION']:
    count = content.count(f'GalaxyCategory.{g}')
