#!/usr/bin/env python3
"""Fix idiom database v2."""

with open('src/data/idioms.ts', 'r', encoding='utf-8') as f:
    content = f.read()
    lines = content.split('\n')

# Strategy: remove ALL entries that are in wrong positions,
# then re-insert everything correctly.

# Find all idiom entry lines (starting with "    { text:")
entry_lines = []
for i, line in enumerate(lines):
    if line.strip().startswith('{ text:'):
        entry_lines.append(i)

print(f"Found {len(entry_lines)} idiom entries")

# Parse each entry to determine its galaxy
import re
entries = []
for i, line in enumerate(lines):
    s = line.strip()
    if s.startswith('{ text:'):
        # Find the galaxy
        galaxy_match = re.search(r'GalaxyCategory\.(\w+)', s)
        if galaxy_match:
            entries.append((i, galaxy_match.group(1), line))
        else:
            # multi-line entry? check next lines
            for j in range(1, 5):
                if i+j < len(lines):
                    combined = s + ' ' + lines[i+j].strip()
                    galaxy_match = re.search(r'GalaxyCategory\.(\w+)', combined)
                    if galaxy_match:
                        entries.append((i, galaxy_match.group(1), line))
                        break

print(f"Parsed {len(entries)} entries")
for g in ['HISTORY', 'MYTHOLOGY', 'NATURE', 'PHILOSOPHY', 'EMOTION']:
    count = sum(1 for _, galaxy, _ in entries if galaxy == g)
    print(f"  {g}: {count}")

# The original 100 entries + new entries. We want 30 per galaxy.
# Current EMOTION has 41 (20 original + 21 new from all galaxies)
# Other galaxies have 20-21 each (original only)

# Remove entries from EMOTION that don't belong there
# Keep only entries with GalaxyCategory.EMOTION
# The extras need to go to their proper galaxies

# Simplify: remove ALL entries, reconstruct with correct distribution
# Find the start and end of the rawIdioms array
array_start = None
array_end = None
for i, line in enumerate(lines):
    if 'const rawIdioms' in line and '[' in line:
        array_start = i
    if array_start and i > array_start and line.strip() == '];':
        array_end = i
        break

print(f"Array: lines {array_start}-{array_end}")

# Keep all original entries that are correctly placed
# But remove ALL extra entries (anything after the original last entry of each section)

# Let's find the section comment lines
section_comments = {}
for i, line in enumerate(lines):
    if '═══' in line and ('历史典故' in line or '神话传说' in line or
                          '自然风物' in line or '为人处世' in line or '情感心理' in line):
        for name in ['历史典故', '神话传说', '自然风物', '为人处世', '情感心理']:
            if name in line:
                section_comments[name] = i
                break

print("Section comments:", section_comments)

# Map section names to galaxy keys
name_to_galaxy = {
    '历史典故': 'HISTORY',
    '神话传说': 'MYTHOLOGY',
    '自然风物': 'NATURE',
...
# Remove all extra entries (non-original) from the wrong positions
# Keep only original 20 per galaxy, then add new 10 in correct position

# First, let's find lines between last original entry of each galaxy and next section comment
# Original entries end at specific lines - the ones that have "galaxy: GalaxyCategory.XXX },"

# Remove entries that are in the wrong galaxy
# The extra entries (from first script) are all in EMOTION section after original emotions
# We need to:
# 1. Remove the 40 extra entries from EMOTION (the ones from HISTORY, MYTHOLOGY, NATURE, PHILOSOPHY)
# 2. Insert 10 new entries at the end of each of the 4 other sections

# Find the line before the closing bracket
closing_bracket = None
for i in range(len(lines) - 1, 0, -1):
    if lines[i].strip() == '];' and i > 160:
        closing_bracket = i
        break

print(f"Closing bracket at line {closing_bracket}")

# Remove all extra entries that were inserted (everything that's not original)
# These are the 50 new entries that all landed at the end of EMOTION
# Remove them by deleting all lines from the last original EMOTION entry + 1 to closing_bracket

# Find the last original EMOTION entry (热血沸腾)
last_orig_emotion = None
for i in range(closing_bracket - 1, 0, -1):
    if '热血沸腾' in lines[i]:
        last_orig_emotion = i
        break

print(f"Last original EMOTION entry at line {last_orig_emotion}")

# Delete everything between last_orig_emotion+1 and closing_bracket (exclusive)
del lines[last_orig_emotion + 1:closing_bracket]

# Now insert new entries in correct positions
# For each galaxy (except EMOTION), find the last entry before the next section comment
offset = 0
for section_name in ['历史典故', '神话传说', '自然风物', '为人处世']:
    galaxy = name_to_galaxy[section_name]
    next_name = next_section_map[section_name]

    # Find the next section's comment line
    insert_at = None
    for i in range(len(lines)):
        if next_name in lines[i] and '═══' in lines[i]:
            insert_at = i
            break

    if insert_at:
        # Insert new entries before the next section comment
        new_entries = new_idioms_data[galaxy]
        for entry in reversed(new_entries):
            lines.insert(insert_at, entry)
        print(f"Inserted {len(new_entries)} entries for {galaxy} before line {insert_at} ({next_name})")

# For EMOTION, insert before `  ];`
# Find closing bracket again
closing_bracket = None
for i in range(len(lines) - 1, 0, -1):
    if lines[i].strip() == '];' and 'rawIdioms' not in lines[i]:
        closing_bracket = i
        break

print(f"New closing bracket at line {closing_bracket}")

# Insert EMOTION entries before closing bracket
new_entries = new_idioms_data['EMOTION']
for entry in reversed(new_entries):
    lines.insert(closing_bracket, entry)
print(f"Inserted {len(new_entries)} EMOTION entries")

# Update header and limits
new_content = '\n'.join(lines)
new_content = new_content.replace('100 成语数据', '150 成语数据')
new_content = new_content.replace('5 星系 × 20 成语', '5 星系 × 30 成语')
for name in ['历史典故', '神话传说', '自然风物', '为人处世', '情感心理']:
    new_content = new_content.replace(f'═══ {name} (20) ═══', f'═══ {name} (30) ═══')
new_content = new_content.replace('限制总连线数不超过 200 条', '限制总连线数不超过 300 条')
# Fix the connection limit checks (there are two)
new_content = new_content.replace('connections.length >= 200)', 'connections.length >= 300)')
# Also fix the condition in the loop check
import re
# Find all occurrences of >= 200 in the file
new_content = re.sub(r'(\w+)\.length >= 200', r'\1.length >= 300', new_content)
...
# Verify
for g in ['HISTORY', 'MYTHOLOGY', 'NATURE', 'PHILOSOPHY', 'EMOTION']:
    count = new_content.count(f'GalaxyCategory.{g}')
    print(f'{g}: {count} idioms')
total = new_content.count('{ text:')
print(f'Total: {total} idioms')
print('Done!')
